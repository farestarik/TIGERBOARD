...
if(!function_exists('redirectWithError')){
    function redirectWithError($message, $route = null){
...
if(!function_exists('getPermissionData')){
    function getPermissionData(array $actions, $name){
        $output = [];

        foreach($actions as $action){
            $output[] = [
                'name' => strtolower($action) . "_" . strtolower($name),
                'display_name' => ucfirst($action) . ' ' . ucfirst($name),
                'description' => ucfirst($action) . ' ' . ucfirst($name),
            ];
        }
        return $output;
    }
...
if(!function_exists('crudsa')){
    function crudsa(...$addons){
        $default = ['create',"read",'update','delete', "import", "export"];
...
if(!function_exists('generateCodeOld')){

    function generateCodeOld($length = 5, $prefix = null){
        if(is_null($prefix)){ $prefix = ''; }
        $nums = '1234567890ABCDEFGHIJKLMNOPQRSTUVWXYZ';
...
if (!function_exists('generateCode')) {
    ...
    function generateCode($length = 5, $prefix = null, $table = null, $column = 'code') {
        if (is_null($prefix)) {
            $prefix = '';
        }
        $nums = '1234567890ABCDEFGHIJKLMNOPQRSTUVWXYZ';
        ...
        do {
            ...
            // Check uniqueness in the database if table and column are provided
            $isUnique = true; // Assume it's unique
            if ($table && $column) {
                $isUnique = @!DB::table($table)->where($column, $code)->exists();
            }
        } while (!$isUnique);
