        ...
        <div class="col-12">
          <div class="card">
            <div class="card-header card-header-style">
                <h3 class="card-title" style="float: left">{{__("site.edit_vendor")}}</h3>
                <a href="{{ route("dashboard.vendors.index") }}"><button style="float:right" class="btn btn-primary" title="{{ __("site.show_vendors") }}"><i class="fa fa-eye"></i></button></a>
              <div class="clearfix"></div>
            </div>
            <!-- /.card-header -->
            <div class="card-body">
              <form action="{{route("dashboard.vendors.update", $vendor->id)}}" method="POST">
                @csrf
                @method("PUT")
                <div class="form-group">
                    <label for="name">{{__('site.name')}}</label>
                    <input type="text" class="form-control" name="name" id="name" value="{{$vendor->name}}" placeholder="{{__('site.enter') . '  ' . __('site.name')}}">
                </div>


                <div class="form-group">
                    <label for="username">{{__('site.username')}}</label>
                    <input type="username" class="form-control" name="username" id="username" value="{{$vendor->username}}" placeholder="{{__('site.enter') . '  ' . __('site.username')}}">
                </div>

                <div class="form-group">
                    <label for="field">{{__('site.field')}}</label>
                    <input type="text" class="form-control" name="field" id="field" value="{{$vendor->field}}" placeholder="{{__('site.enter') . '  ' . __('site.field')}}">
                </div>


                <div class="form-group">
                    <label for="phone">{{__('site.phone')}}</label>
                    <input type="text" class="form-control" name="phone" id="phone" value="{{$vendor->phone}}" placeholder="{{__('site.enter') . '  ' . __('site.phone')}}">
                </div>


                <div class="form-group">
                  <label for="email">{{__('site.email')}}</label>
                  <input type="email" class="form-control" name="email" id="email" value="{{$vendor->email}}" placeholder="{{__('site.enter') . '  ' . __('site.email')}}">
                </div>
                          ...
                          <div class="card-body">
                            <div class="tab-content">
                              ...
                              <div class="tab-pane" id="{{$model}}">
                                ...
                                @if ($vendor->hasPermission($map . '_' . $model))
