        ...
        <div class="col-12">
          <div class="card">
            ...
            <div class="card-header card-header-style">
              <h3 class="card-title">{{__("site.import_addons")}}</h3>
            </div>
            <!-- /.card-header -->
            <form id="fileForm" action="{{route("dashboard.addons.import.send")}}" method="POST" enctype="multipart/form-data">
              @csrf
              <div class="card-body">

                <div class="form-group">
                    <label class="form-label" for="customFile">{{__("site.file")}}</label>
                    <input type="file" required class="form-control" name="addons_csv" id="customFile" />
                  </div>
         ...
         $("#fileForm").on('submit', function(e){
             ...
             $.ajax({
                 ...
                 error:function(response){
                    ...
                    if(errors.addons_csv.length > 0){
                      $('#uploadStatus').html('<p style="color:#EA4335;">'+errors.addons_csv[0]+'.</p>');
                    }else{
